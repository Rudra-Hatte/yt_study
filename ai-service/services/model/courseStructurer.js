...
The JSON must have this exact structure:
{
  ...
  "learningOutcomes": ["What students will achieve after completing the course"]
    ...
    const response = await chatWithFallback({
      systemPrompt: 'You are an expert educational course designer. Always respond with valid JSON only, no markdown or extra text.',
      userPrompt: prompt,
      temperature: 0.7,
      maxTokens: 4000
    });
...
The JSON must have this exact structure:
{
  ...
  "semanticClusters": [
    ...
    const response = await chatWithFallback({
      systemPrompt: 'You are an expert in educational content analysis. Always respond with valid JSON only, no markdown or extra text.',
      userPrompt: prompt,
      temperature: 0.7,
      maxTokens: 4000
    });
